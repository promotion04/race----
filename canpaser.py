...
base_directory = 'C:\\Users\\SEUNGJIN\\Desktop\\can'
...
for folder in Org_file_list:
    ...
    for directory in directories:
        ...
        ############################
        f_amk_setpoint1_fl = open(file_dst + directory + '_amk_setpoint1_fl.csv', 'w', newline='')
        f_amk_setpoint1_fr = open(file_dst + directory + '_amk_setpoint1_fr.csv', 'w', newline='')
        ...
        writer.writerow(['time', 'AMK_TempMotor', 'AMK_TempInverter', 'AMK_ErrorInfo', 'AMK_TempIGBT'])
        ##############################
        writer = csv.writer(f_amk_setpoint1_fl)
        writer.writerow(['time', 'AMK_bInverterOn', 'AMK_bDcOn', 'AMK_bEnable', 'AMK_bErrorReset', 'AMK_Torque_setpoint', 'AMK_TorqueLimitPositv', 'AMK_TorqueLimitNegativ'])
        writer = csv.writer(f_amk_setpoint1_fr)
        writer.writerow(['time', 'AMK_bInverterOn', 'AMK_bDcOn', 'AMK_bEnable', 'AMK_bErrorReset', 'AMK_Torque_setpoint', 'AMK_TorqueLimitPositv', 'AMK_TorqueLimitNegativ'])
        ...
        writer.writerow(['time', 'steering_angle ', 'apps', 'bpps', 'brake_pressure'])
        ...
        for file in files:
            ...
            for line in lines:
                ...
                if line[3] == '00040300':  # CAN ID가 0x40300인 경우
                    # CAN 데이터는 총 8바이트여야 함 (line[4] ~ line[11])
                    if len(line) < 12:
                        continue
                    
                    # 각 필드 추출 (big-endian 방식)
                    steering_angle = (int(line[4], 16) << 8) | int(line[5], 16)  
                    apps = int(line[6], 16)                                        
                    bpps = int(line[7], 16)                                        
                    brake_pressure = (int(line[8], 16) << 8) | int(line[9], 16)               
        ...
        ############
        f_amk_setpoint1_fl.close()
        f_amk_setpoint1_fr.close()
